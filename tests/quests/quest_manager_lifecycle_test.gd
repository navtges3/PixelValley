...
func _test_invalid_transitions() -> void:
	...
	_expect_true(not manager.abandon_quest(main_quest), "active main quest cannot be abandoned")

	var duplicate := _make_side_quest(main_quest.id)
	_expect_true(not manager.offer_quest(duplicate), "duplicate tracked quest ID cannot be offered")
	_expect_true(not manager.mark_quest_ready(main_quest), "quest with unmet objectives cannot become ready")
