...
func run_tests() -> int:
	...
	_test_ready_and_completed_quests_cannot_be_reactivated()
	_test_duplicate_quest_id_cannot_be_activated()
	_cleanup()
...
func _test_ready_and_completed_quests_cannot_be_reactivated() -> void:
	var manager := QuestManager.new()
	GameState.quest_manager = manager
	var ready_quest := _make_side_quest(109)
	manager.activate_quest(ready_quest)
	var ready_objective := ready_quest.objectives[0] as KillQuestObjective
	ready_objective.current_amount = ready_objective.target_amount
	_expect_true(manager.mark_quest_ready(ready_quest), "completed objectives move the quest into the ready state")
	_expect_true(manager.is_quest_ready(ready_quest.id), "the test quest begins in the ready state")
	var reactivated_ready := manager.activate_quest(ready_quest)
	_expect_true(not reactivated_ready, "a ready quest cannot be reactivated")
	_expect_true(manager.is_quest_ready(ready_quest.id), "a rejected ready-quest activation preserves the ready state")
	_expect_true(not manager.is_quest_active(ready_quest.id), "a rejected ready-quest activation does not add it to active quests")
	var completed_quest := _make_side_quest(110)
	completed_quest.completed = true
	manager.completed_quests.append(completed_quest)
	var reactivated_completed := manager.activate_quest(completed_quest)
	_expect_true(not reactivated_completed, "a completed quest cannot be reactivated")
	_expect_true(manager.is_quest_completed(completed_quest.id), "a rejected completed-quest activation preserves the completed state")
	_expect_true(not manager.is_quest_active(completed_quest.id), "a rejected completed-quest activation does not add it to active quests")
	_expect_true(completed_quest.completed, "rejected activation does not clear the quest completion flag")


func _test_duplicate_quest_id_cannot_be_activated() -> void:
	var manager := QuestManager.new()
	GameState.quest_manager = manager
	var original_quest := _make_side_quest(111)
	manager.locked_quests.append(original_quest)
	var duplicate_quest := _make_side_quest(original_quest.id)
	var activated_duplicate := manager.activate_quest(duplicate_quest)
	_expect_true(not activated_duplicate, "a separate quest resource with a registered ID cannot be activated")
	_expect_true(original_quest in manager.locked_quests, "rejecting duplicate activation preserves the original quest state")
	_expect_true(duplicate_quest not in manager.active_quests, "the duplicate quest is not added to active quests")
	_expect_equal(manager.active_quests.size(), 0, "duplicate activation does not modify the active quest list")
	_expect_true(manager.get_quest_by_id(original_quest.id) == original_quest, "quest lookup still resolves to the originally registered quest")
